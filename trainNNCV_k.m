function theta=trainNNCV_k(net,sys,par,data,w0,numIterationsPerEpoch)
...
Loss_s = norm(b_s-c_s);%+10*norm(dcdu - 1);
...
for i=1:length(data)
    for j = 1:size(data(i).u,2)
        ...
        Loss = Loss + 1/N*sum(LossFunc((-1e-3+1e-5:1e-4:1e-3)*L,u+(-1e-3+1e-5:1e-4:1e-3),repmat(data(i).y(1:par.nx,j),1,20),w));
    end
end
...
if nargin<5 || isempty(w0) 
    w0 = [10*ones(num_u,1);randn(num_theta,1)];
end
...
while i < numIterationsPerEpoch && ~monitor.Stop
    ...
    if mod(i,10)==0
        ...
        recordMetrics(monitor,iteration,Loss=log(full(loss)));
