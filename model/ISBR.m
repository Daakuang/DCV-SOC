...
sys.L_path = (c_C*V<n_Cdes);%tf;%+0.1*m_dot_f^2+0.02*T_in_M^2+0.01*T_in_EK^2;
sys.L_terminal=0;
sys.isTFfree = 2;  % 1:state cost 2: minimum time 3:state cost + minimum time

sys.nlcon = vertcat(u,T_cf,V,c_C*V);
sys.lb = vertcat(u_min,-Inf,-Inf,n_Cdes);
sys.ub = vertcat(u_max,T_max,V_max,inf);  
sys.con_path=[1 2];
