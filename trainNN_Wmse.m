function [theta,Losses,Grades]=trainNN_Wmse(net,sol,Luu,w0,miniBatchSize,numEpochs)
...
Loss_i = cellfun(@(sol,Luu)(sol.x(num_Xi+1)-nn(sol.x(1:num_Xi),w))'*Luu*(sol.x(num_Xi+1)-nn(sol.x(1:num_Xi),w)),sol,Luu,UniformOutput=false);
Loss_F = 0;
for i=1:length(sol)
...
mseLoss=100;
while i < numIterations && ~monitor.Stop && mseLoss>=0.01
    epoch = epoch + 1;
    ...
    for j =1:numIterationsPerEpoch
        ...
        if mod(i,10)==0
            loss = f(theta);
            % mseLoss = mean( cellfun(@(sol)full((sol.x(num_Xi+1)-nn(sol.x(1:num_Xi),theta)).^2),sol));
            k = k+1;
            Losses(k)=full(loss);
